...
Name: Eric Schrock
Email: eric.schrock22@gmail.com
...
PNG createSpotlight(PNG image, int centerX, int centerY) {
  for (unsigned x = 0; x < image.width(); x++) {
    for (unsigned y = 0; y < image.height(); y++) {
      ...
      double deltaX = std::abs(centerX - (int)x);
      double deltaY = std::abs(centerY - (int)y);

      double euclideanDistance = std::sqrt((deltaX * deltaX) + (deltaY * deltaY));

      if (euclideanDistance >= 160) {
        pixel.l *= 0.2;
      } else {
        pixel.l *= (100 - (euclideanDistance / 2)) / 100;
      }
    }
  }
...
PNG illinify(PNG image) {
  for (unsigned x = 0; x < image.width(); x++) {
    for (unsigned y = 0; y < image.height(); y++) {
      ...
      if ((pixel.h >= (illiniBlue - (hueRange / 4))) &&
          (pixel.h <  (illiniBlue + (hueRange / 4)))) {
        pixel.h = illiniBlue;
      } else {
  ...
  for (unsigned x = 0; x < firstImage.width() && x < secondImage.width(); x++) {
    for (unsigned y = 0; y < firstImage.height() && y < secondImage.height(); y++) {
      ...
      if (secondPixel.l == 1.0) {
        ...
        firstPixel.l = (firstPixel.l > 1.0) ? 1.0 : firstPixel.l;
      }
    }
  }
