...
static void treeFactory(GenericTree<int>& tree) {
  ...
  GenericTree<int>::TreeNode* child = root->addChild(8);
  GenericTree<int>::TreeNode* grandchild = child->addChild(16);

  grandchild->addChild(42);
  child->addChild(23);
  root->addChild(15);
}
...
std::vector<T> traverseLevels(GenericTree<T>& tree) {
  ...
  parents.push_back(rootNodePtr);

  while (parents.size() > 0) {
    for (auto parent : parents) {
      results.push_back(parent->data);

      for (auto child : parent->childrenPtrs) {
        if (child) {
